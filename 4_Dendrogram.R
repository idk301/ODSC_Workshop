...
top.words$word<-factor(top.words$word, 
                       levels=unique(as.character(top.words$word)))
ggplot(top.words, aes(x=word, y=frequency)) +
...
ggplot(a.df, aes(y=terms)) + 
  geom_point(aes(x=zombie.assoc), data=a.df) +
  theme_gdocs() + 
  geom_text(aes(x=zombie.assoc,label=zombie.assoc), colour="red",hjust=-.25)
